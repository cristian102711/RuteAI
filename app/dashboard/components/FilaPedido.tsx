    ...
    <div className="flex justify-between items-center p-4 rounded-xl bg-zinc-950 border border-zinc-800 hover:border-zinc-700 transition group">
      <div>
        <h3 className="font-bold text-sm uppercase tracking-wider text-zinc-200 flex items-center gap-2">
          {pedido.producto} 
          {pedido.repartidor && (
            <span className="lowercase normal-case text-xs font-normal px-2 py-0.5 bg-blue-500/10 text-blue-400 rounded-full border border-blue-500/20 flex items-center gap-1">
              <UserCircle2 className="w-3 h-3"/> {pedido.repartidor.nombre}
            </span>
          )}
        </h3>
      ...
      <div className="flex items-center gap-3">
        ...
        {pedido.estado === "pendiente" && repartidores.length > 0 && (
          <select 
            className="bg-zinc-900 text-xs border border-zinc-700 text-zinc-300 rounded-md px-2 py-1 outline-none focus:border-emerald-500 w-32"
            value={pedido.repartidorId || "unassigned"}
            onChange={async (e) => {
              const loadingId = toast.loading("Asignando repartidor...");
              await asignarRepartidor(pedido.id, e.target.value);
              toast.success("Repartidor asignado", { id: loadingId });
            }}
          >
