...
export default function DashboardScreen() {
  ...
  // Próxima parada simplificada: primer pedido pendiente
  const proximaParada = pendientes[0];
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.date}>{new Date().toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' })}</Text>
      </View>
        ...
        <Card style={styles.nextStopCard}>
          <View style={styles.nextStopHeader}>
            ...
            <View style={styles.nextStopInfo}>
              <Text style={styles.clientName}>{proximaParada.nombreCliente}</Text>
              <Text style={styles.address}>{proximaParada.direccion}</Text>
            </View>
            <RiskBadge score={proximaParada.scoreRiesgo} />
          </View>
          <View style={styles.etaContainer}>
            <Text style={styles.etaLabel}>ETA Estimado</Text>
            <Text style={styles.etaValue}>En 15 min</Text>
          </View>
        </Card>
      ) : (
        <Card style={styles.emptyCard}>
          <Text style={styles.emptyText}>No hay paradas próximas</Text>
        </Card>
      )}
    </ScrollView>
  );
