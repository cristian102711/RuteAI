...
export function EliminarEmpresaButton({ id, nombre }: { id: string; nombre: string }) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast.success(`${nombre} eliminada permanentemente`, { id: toastId });
    } catch {
      toast.error("Error al eliminar la empresa", { id: toastId });
    } finally {
      setLoading(false);
      setConfirm(false);
    }
        ...
        <div className="bg-zinc-900 border border-red-500/30 rounded-2xl p-8 max-w-sm w-full shadow-2xl">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-zinc-400">
              Esto eliminará <span className="text-white font-semibold">{nombre}</span> y todos sus datos permanentemente. Esta acción no se puede deshacer.
            </p>
          </div>
  ...
  return (
    <button onClick={() => setConfirm(true)} title="Eliminar empresa"
