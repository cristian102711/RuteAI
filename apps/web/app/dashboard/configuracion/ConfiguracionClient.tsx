...
import { actualizarEmpresa, actualizarPerfil } from "../actions";
...
export default function ConfiguracionClient({ initialEmpresa, initialUsuario }: ConfiguracionClientProps) {
  const [activeTab, setActiveTab] = useState<TabId>("empresa");

  // --- ESTADOS DE CONFIGURACIÓN CON DATOS REALES ---
  // Empresa
  const [nombreEmpresa, setNombreEmpresa] = useState(initialEmpresa.nombre);
  const [rutEmpresa, setRutEmpresa] = useState("900.482.193-4"); // Mockup
  const [pais, setPais] = useState("Colombia"); // Mockup
  const [zonaHoraria, setZonaHoraria] = useState("America/Bogota (GMT-5)"); // Mockup
  const [direccion, setDireccion] = useState("Cra. 11 #93-46, Bogotá D.C."); // Mockup
  ...
  const handleGuardarEmpresa = async () => {
    try {
      const res = await actualizarEmpresa(nombreEmpresa, emailSoporte);
      if (res.success) {
        toast.success("Empresa actualizada con éxito", {
          description: "La información comercial ha sido sincronizada en la base de datos.",
        });
      } else {
        toast.error("Error al actualizar empresa: " + res.error);
      }
    } catch (err) {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={email}
                    disabled
                    className="w-full mt-2 bg-zinc-950/40 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-zinc-500 cursor-not-allowed outline-none"
                  />
                </div>
                <div>
                ...
                <div className="w-[280px] aspect-[9/18] bg-zinc-900 border-[6px] border-zinc-800 rounded-[2.5rem] relative flex flex-col overflow-hidden shadow-2xl">
                  ...
                  <div className="flex-1 flex flex-col pt-5 bg-zinc-950">
                    <div className="bg-zinc-900 border-b border-zinc-800 px-3 py-2 text-[10px] font-bold text-zinc-300 text-center">
                      {canal === "whatsapp" ? "💬 RuteAI Logística" : "📱 " + telefono}
                    </div>
