...
import { Plus, Phone, Truck, MapPin } from "lucide-react";
  ...
  return (
    <div className="space-y-6 p-6 lg:p-8 max-w-7xl mx-auto text-zinc-100 selection:bg-purple-500/30">
      ...
      <div className="flex items-end justify-between">
        ...
        <button className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-purple-600 to-amber-500 px-4 py-2 text-sm font-semibold text-white shadow-[0_0_15px_rgba(245,158,11,0.2)] hover:opacity-90 transition-opacity">
          ...
          <tbody className="divide-y divide-zinc-800">
            {repartidores.length === 0 ? (
              ...
              repartidores.map((rep, idx) => {
                ...
                // Usar datos reales si existen, sino mock exacto de Lovable
                const telefono = rep.telefono ?? mock.tel;
                const totalPedidos = rep._count.pedidosAsignados > 0 ? rep._count.pedidosAsignados : mock.pedidos;
                
                // Calculo real vs mock
                const progreso = rep._count.pedidosAsignados > 0 ? `${Math.min(100, totalPedidos * 10)}%` : mock.progreso;
                const estaActivo = rep._count.pedidosAsignados > 0;
                
                const estadoTxt = estaActivo ? "Activo" : mock.estado;
                const estadoCls = estaActivo ? mockData[0].estadoClasses : mock.estadoClasses;
                  ...
                  <tr key={rep.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-3 text-zinc-400">
                      <div className="flex items-center gap-1.5">
                        <Truck className="h-3.5 w-3.5" /> {mock.vehiculo}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3 text-zinc-400">
                      <div className="flex items-center gap-1.5">
                        <MapPin className="h-3.5 w-3.5" /> {mock.ubicacion}
                      </div>
                    </td>
