...
import { MapPickerInput } from "../../components/MapPickerInput";

const INPUT_CLASS =
  "bg-zinc-950/80 border border-white/[0.04] rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-amber-500/30 focus:border-amber-500/50 text-white transition-all outline-none w-full";
...
export default function CrearPedidoPage() {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Error al crear", { id: toastId });
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="max-w-2xl mx-auto">
        <header className="mb-8 flex flex-col border-b border-white/[0.04] pb-6">
          <Link
            ...
            className="flex items-center gap-2 text-zinc-500 hover:text-white transition-colors mb-4 text-sm font-medium w-fit"
          >
        ...
        <section className="bg-zinc-900/40 backdrop-blur-md border border-white/[0.04] rounded-3xl p-8 shadow-xl">
          <form action={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="grid gap-1.5">
              ...
              {/* Hidden inputs para que FormData reciba los valores controlados */}
