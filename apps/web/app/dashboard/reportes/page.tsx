...
const PRECIO_LITRO_CLP = 1100;
const KG_CO2_POR_LITRO = 2.38;
const HORAS_AHORRADAS_POR_ENTREGA = 0.15;
...
export default async function ReportesPage() {
  ...
  const pedidosSeisMeses = await prisma.pedido.findMany({
    where: { empresaId, createdAt: { gte: seisMesesAtras } },
    select: { id: true, estado: true, motivoFallo: true, createdAt: true },
  });
  ...
  const tasaExito = totalHoy > 0 ? Math.round((entregadosHoy / totalHoy) * 100) : 0;

  const totalEntregados6m = pedidosSeisMeses.filter((p) => p.estado === "entregado").length;
  ...
  const graficoMeses = Array.from({ length: 6 }).map((_, i) => {
    ...
    const costSinIA = totalMes * LITROS_POR_ENTREGA * 1.35 * PRECIO_LITRO_CLP;
  ...
  const pedidosFallidos = pedidosSeisMeses.filter((p) => p.estado === "fallido");
  ...
  const kpiCards = [
    {
      ...
      value: `${litrosAhorrados} L`,
      sub: `≈ $${ahorroClp.toLocaleString("es-CL")} CLP ahorrado`,
    },
    {
      icon: Clock,
      label: "Tiempo recuperado",
      value: `${horasRecuperadas} h`,
      sub: "vs rutas no optimizadas · 6 meses",
    },
    {
      icon: TrendingUp,
      label: "Tasa de éxito",
      value: `${tasaExito6m}%`,
      sub: `${totalEntregados6m} de ${pedidosSeisMeses.length} pedidos entregados`,
    },
    {
      icon: Leaf,
      label: "CO₂ evitado",
      value: `${co2Evitado} kg`,
      sub: `equiv. ${arbolesEquivalentes} árbol${arbolesEquivalentes !== 1 ? "es" : ""}`,
    },
  ];
  ...
  return (
    <PageWrapper className="space-y-6 p-6 lg:p-8 max-w-7xl mx-auto text-zinc-100 selection:bg-purple-500/30">
      ...
      <AnimatedHeader className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="text-xs uppercase tracking-widest text-amber-500 font-bold">Analítica</div>
      ...
      {/* Grid 4 KPIs — stagger + hover lift */}
      <AnimatedGrid className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpiCards.map(({ icon: Icon, label, value, sub }) => (
          <AnimatedCard
            key={label}
            className="rounded-xl border border-white/[0.04] bg-white/[0.02] p-5 cursor-default"
          >
      ...
      <AnimatedSection delay={0.15} className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-white/[0.04] bg-white/[0.02] p-5 lg:col-span-2">
          ...
          {pedidosSeisMeses.length === 0 ? (
            <div className="mt-6 flex h-52 items-center justify-center text-center">
              <p className="text-zinc-500 text-sm">Sin datos de pedidos en los últimos 6 meses.</p>
            </div>
          ) : (
            <div className="mt-6 flex h-52 items-end gap-4">
              {graficoNormalizado.map((mes) => (
                <div key={mes.nombre} className="flex flex-1 flex-col items-center gap-2 group cursor-pointer">
        ...
        <div className="rounded-xl border border-white/[0.04] bg-white/[0.02] p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-zinc-500 font-mono">{totalFallidos} fallidos</span>
          </div>

          {motivosFallo.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              <span className="text-2xl mb-2">🎉</span>
              <p className="text-sm text-zinc-400 font-medium">Sin entregas fallidas</p>
              ...
              {motivosFallo.map((motivo) => (
                <li key={motivo.causa} className="group">
                  <div className="flex justify-between text-xs mb-1.5">
                    ...
                    <span className="font-mono text-zinc-500 ml-2 shrink-0">{motivo.percent}%</span>
                  </div>
