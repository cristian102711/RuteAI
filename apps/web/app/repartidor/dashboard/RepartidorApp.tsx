...
function PedidosTab({ pedidos, onRefresh, isRefreshing }: {
  ...
  // ── Flujo operativo SLA: entrega con evidencia, fallo/cancelación con motivo ──
  const [modalEvidencia, setModalEvidencia] = useState<string | null>(null);
  const [motivoSheet, setMotivoSheet] = useState<{ tipo: "fallido" | "cancelado"; pedidoId: string } | null>(null);
  const [confirmando, setConfirmando] = useState<string | null>(null);

  const pedidoModal = useMemo(
    () => pedidos.find((p) => p.id === modalEvidencia) ?? null,
    [pedidos, modalEvidencia]
  );

  async function actualizarEstado(pedidoId: string, estado: "fallido" | "cancelado", motivo?: string) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <ol className="flex-1 overflow-y-auto p-3 space-y-2">
        {pedidosFiltrados.length === 0 ? (
          ...
          pedidosFiltrados.map((pedido, i) => {
            const cfg = ESTADO_CONFIG[pedido.estado] ?? ESTADO_CONFIG.pendiente;
            const { Icon } = cfg;
            const activo = pedido.estado === "pendiente" || pedido.estado === "en_ruta";
            const atrasado = activo && estaAtrasado(pedido);
            return (
              <li key={pedido.id}>
                <div className={`flex items-start gap-3 rounded-2xl border ${atrasado ? "border-rose-500/40" : "border-white/[0.04]"} bg-white/[0.02] p-4 active:bg-white/[0.04] transition-colors`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {atrasado && (
                      <span className="mt-1.5 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-bold bg-rose-500/15 text-rose-400 border border-rose-500/40 animate-pulse">
                        <Clock className="h-2.5 w-2.5" /> ATRASADO · {formatearAtraso(minutosAtraso(pedido))}
                      </span>
                    )}
                    {activo && !atrasado && pedido.fechaEntregaLimite && (
                      <div className="flex items-center gap-1 mt-1.5">
                        <Clock className="h-3 w-3 text-zinc-600" />
                        <span className="text-xs text-zinc-500">
                          Límite: {new Date(pedido.fechaEntregaLimite).toLocaleTimeString("es-CL", { hour: "2-digit", minute: "2-digit", timeZone: "America/Santiago" })}
                        </span>
                      </div>
                    )}
                    ...
                    {/* Acciones operativas (solo pedidos activos) */}
                    {activo && (
                      <div className="mt-3 space-y-2">
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            onClick={() => setModalEvidencia(pedido.id)}
                            disabled={confirmando === pedido.id}
                            className="flex items-center justify-center gap-1.5 rounded-xl bg-emerald-500/20 border border-emerald-500/40 py-2.5 text-xs font-bold text-emerald-400 hover:bg-emerald-500/30 transition-colors disabled:opacity-60"
                          >
                            <CheckCircle2 className="h-4 w-4" /> Entregado
                          </button>
                          <button
                            onClick={() => setMotivoSheet({ tipo: "fallido", pedidoId: pedido.id })}
                            disabled={confirmando === pedido.id}
                            className="flex items-center justify-center gap-1.5 rounded-xl bg-orange-500/20 border border-orange-500/40 py-2.5 text-xs font-bold text-orange-400 hover:bg-orange-500/30 transition-colors disabled:opacity-60"
                          >
                            {confirmando === pedido.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4" />}
                            No entregado
                          </button>
                        </div>
                        <button
                          onClick={() => setMotivoSheet({ tipo: "cancelado", pedidoId: pedido.id })}
                          disabled={confirmando === pedido.id}
                          className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-red-500/30 bg-red-500/10 py-2 text-[11px] font-bold text-red-400 hover:bg-red-500/20 transition-colors disabled:opacity-60"
                        >
      ...
      {modalEvidencia && pedidoModal && (
        <ModalEvidencia
          pedidoId={modalEvidencia}
          nombreCliente={pedidoModal.nombreCliente}
          onClose={() => setModalEvidencia(null)}
          onSuccess={() => { setModalEvidencia(null); onRefresh(); }}
        />
      )}

      {/* Hoja de motivos (no entregado / cancelar) */}
      {motivoSheet && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setMotivoSheet(null)} />
          <div className="relative z-10 w-full sm:max-w-sm bg-zinc-900 sm:rounded-3xl rounded-t-3xl border border-zinc-800 shadow-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-800/60">
              <h2 className="text-base font-bold text-white">
                {motivoSheet.tipo === "fallido" ? "¿Por qué no se entregó?" : "Motivo de cancelación"}
              </h2>
              <p className="text-xs text-zinc-400 mt-0.5">
                {motivoSheet.tipo === "fallido"
                  ? "Quedará como intento fallido (re-agendable)."
                  : "El pedido se anulará y no podrá cambiar de estado."}
              </p>
            </div>
            <div className="p-3 space-y-1.5">
              {(motivoSheet.tipo === "fallido" ? MOTIVOS_FALLO : MOTIVOS_CANCELACION).map((m) => (
                <button
                  key={m.valor}
                  onClick={() => actualizarEstado(motivoSheet.pedidoId, motivoSheet.tipo, m.valor)}
